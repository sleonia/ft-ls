...
static bool	directory_to_ignore(t_file *file)
{
...
static void fill_files_inside_dir(t_file *file)
{
	t_file *file_counter;
	bool	done;
	...
	while(!done)
	{
		...
		fill_file(file_counter->dirent->d_name, file_counter);
		file_counter = new_file(file_counter);
		file_counter->origin = file;
	}
}

static void fill_directory(t_file *file, const char *name)
{
	t_file 			*file_counter;
	...
	if (!directory_to_ignore(file_counter))
		fill_files_inside_dir(file_counter);
}
...
void	fill_file(const char *name, t_file *file)
{
	...
	if (S_ISDIR(file_stat.st_mode))
		fill_directory(file, file->full_path);
	else
 ...
 * PARSE THIS LOL
 */
static bool has_args(args, ac/*or whatever else is needed maybe flags i don't know*/)
{
